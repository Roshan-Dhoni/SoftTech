...
function reset() {
    ...
    // Remove images from userImageId and cpuImageId
    while (userImageId.firstChild) {
        userImageId.removeChild(userImageId.firstChild);
    }
    while (cpuImageId.firstChild) {
        cpuImageId.removeChild(cpuImageId.firstChild);
    }

    // Re-enable click events on images
    for (let i = 0; i < imgTag.length; i++) {
        imgTag[i].onclick = function() {
        game(i); // Re-attach game logic function with updated index i
        };
    }   
}
